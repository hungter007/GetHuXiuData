...
class Handler(BaseHandler):
	...
	四统一：
	1.项目群同步：测试通过。
	2.基建项目信息：基建项目同步主数据成功后，主数据广播财务系统报错(财务系统导入报文错误：管理对象同步过程出错)。
	3.基建项目单项工程信息：基建项目单项工程同步主数据成功后，主数据广播财务系统报错(财务系统导入报文错误：管理对象同步过程出错)。
	4.同步技改项目信息：技改项目同步主数据成功后，主数据广播财务系统报错(财务系统导入报文错误：管理对象同步过程出错)。
	5.同步修理项目信息：修理项目同步主数据成功后，主数据广播财务系统报错(财务系统导入报文错误：管理对象同步过程出错)。
